...
class SWCClient:
    ...
    Typical usage example:
        ...
        client = SWCClient() 
        response = clent.get_health_check() 
    """ 
    ...
    BULK_FILE_BASE_URL = (
        "https://raw.githubusercontent.com/josefinelantz" + "/api/master/bulk/"
    )

    def __init__(self, input_config: config.SWConfig):
        """Class constructor that sets variables from configuration object."""

        logger.debug(f"Buld file base URL: {self.BULK_FILE_BASE_URL}")
    ...
    def call_api(self, api_endpoint: str, api_params: dict = None) -> httpx.Response: 
        """Makes API call an logs errors"""

        if api_params: 
            # remove any empty params before calling the API
    ...
    def get_health_check(self) -> httpx.Response:
        ...
        Returns:
            An httpx.Response object that contains the HTTP status, JSON response and other information 
            resceived from the API. 
        """
        logger.debug("Entered health check")
        endpoint_url = self.HEALTH_CHECK_ENDPOINT 
        return self.call_api(endpoint_url)
    
    def list_leagues(self, skip: int = 0, limit: int = 100, minimum_last_changed_data: str = None, league_name: str = None) -> List[League]:
        ...
        params = {
            "skip": skip,
            "limit": limit,
            "minimum_last_changed_date": minimum_last_changed_data,
